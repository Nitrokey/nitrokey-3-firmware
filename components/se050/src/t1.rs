...
impl<TWI> T1overI2C<TWI> where
	...
	fn send_s(&mut self, code: T1SCode, data: &[u8], delay: &mut impl DelayMs<u32>) -> Result<(), T1Error> {
		...
		trace!("T1 W S {}", delog::hex_str!(&buf[0..3+data.len()+2]));
	...
	fn receive_s(&mut self, code: T1SCode, data: &mut [u8], delay: &mut impl DelayMs<u32>) -> Result<(), T1Error> {
		...
		trace!("T1 R S H {}", delog::hex_str!(&data[0..3]));
		...
		trace!("T1 R S B {}", delog::hex_str!(&data[0..dlen+2]));
...
impl<TWI> T1Proto for T1overI2C<TWI> where
	...
	fn send_apdu(&mut self, apdu: &CApdu, le: u8, delay: &mut impl DelayMs<u32>) -> Result<(), T1Error> {
		...
		apdubuf[2] = (4 + apdu.data.map_or(0, |d| { d.len()+1 }) + 1) as u8;
		apdubuf[3] = apdu.cla.into();
		apdubuf[4] = apdu.ins;
		apdubuf[5] = apdu.p1;
		apdubuf[6] = apdu.p2;
		let dlen = if let Some(data) = apdu.data {
			if data.len() > 248 { todo!(); }
			apdubuf[7] = data.len() as u8;
			apdubuf[8..8+data.len()].copy_from_slice(data);
			data.len() + 1
		} else { 0 };
		apdubuf[7+dlen] = le;
		let crc = crc16_ccitt_oneshot(&apdubuf[0..8+dlen]);
		set_u16_le(&mut apdubuf[8+dlen..8+dlen+2], crc);

		self.iseq_snd ^= 1;
		trace!("T1 W I {}", delog::hex_str!(&apdubuf[0..8+dlen+2]));
		self.twi_write(&apdubuf[0..8+dlen+2], delay)
	}
	...
	fn receive_apdu<'a, 'b>(&mut self, buf: &'b mut [u8], apdu: &'a mut RApdu<'b>, delay: &mut impl DelayMs<u32>) -> Result<(), T1Error> {
		...
		trace!("T1 R I H {}", delog::hex_str!(&buf[0..3]));
		...
		trace!("T1 R I B {}", delog::hex_str!(&buf[0..dlen+2]));
