...
use lpc55_hal::{drivers::Timer, Anactrl, Iocon, Pmc, Syscon};
...
pub fn init_bootup(iocon: &mut lpc55_pac::IOCON) {
    unsafe {
        types::DEVICE_UUID.copy_from_slice(&lpc55_hal::uuid());
    };
...
pub fn init_rgb(
    syscon: &mut lpc55_hal::Syscon,
    iocon: &mut lpc55_hal::Iocon<Enabled>,
    ctimer: board::PwmTimer,
    clocks: &mut lpc55_hal::drivers::clocks::Clocks,
) -> board::led::RgbLed {
    #[cfg(any(feature = "board-lpcxpresso55"))]
    {
        board::led::RgbLed::new(
            lpc55_hal::drivers::Pwm::new(
                ctimer.enabled(syscon, clocks.support_1mhz_fro_token().unwrap()),
            ),
            iocon,
        )
    }
...
pub fn setup_fm11nc08(
    ...
    nfc_irq: lpc55_hal::Pin<
        nfc::NfcIrqPin,
        lpc55_hal::typestates::pin::state::Gpio<direction::Input>,
    >,
    delay_timer: &mut DelayTimer,
) -> Option<nfc::NfcChip> {
    ...
    nfc::try_setup(spi, gpio, iocon, nfc_irq, delay_timer, force_nfc_reconfig)
}
