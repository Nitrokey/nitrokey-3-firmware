...
mod app {
    ...
    type RtcMonotonic = nrf52840::rtic_monotonic::RtcMonotonic;
    ...
    fn init(mut ctx: init::Context) -> (SharedResources, LocalResources, init::Monotonics) {
        ...
        nrf52840::init_bootup(&ctx.device.FICR, &ctx.device.UICR, &mut ctx.device.POWER);
        ...
        let usbd_ref = {
            if powered_by_usb {
                Some(nrf52840::setup_usb_bus(ctx.device.CLOCK, ctx.device.USBD))
            } else {
                None
            }
        };
