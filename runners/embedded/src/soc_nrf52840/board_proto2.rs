...
pub fn init_pins(gpiote: &Gpiote, gpio_p0: p0::Parts, gpio_p1: p1::Parts) -> BoardGPIO {
	...
	gpiote.channel3().input_pin(&btn4).toggle().enable_interrupt();
...
pub fn power_off() {
	...
	unsafe {
		...
		pac.QSPI.cinstrconf.write(|w| w.opcode().bits(0xb9)
				.length()._1b().wipwait().clear_bit()
				.wren().clear_bit().lfen().clear_bit()
				.lfstop().clear_bit() );
		loop {
	...
	// display
	unsafe { pac.P0.outset.write(|w| w.bits(1u32 << 13)); }	// POWER
	unsafe { pac.P0.outset.write(|w| w.bits(1u32 << 8)); } // BACKLIGHT
	unsafe { pac.P0.outclr.write(|w| w.bits(1u32 << 0)); } // MOSI
	unsafe { pac.P0.outclr.write(|w| w.bits(1u32 << 1)); } // CLK
	unsafe { pac.P0.outclr.write(|w| w.bits(1u32 << 4)); } // RESET
	unsafe { pac.P0.outclr.write(|w| w.bits(1u32 << 6)); } // CS
	unsafe { pac.P0.outclr.write(|w| w.bits(1u32 << 26)); } // DC
	// fingerprint reader
	unsafe { pac.P0.outset.write(|w| w.bits(1u32 << 15)); } // POWER
	unsafe { pac.P1.pin_cnf[9].write(|w| w.bits(2) ); } // DETECT
	unsafe { pac.P0.pin_cnf[11].write(|w| w.bits(2) ); } // RX
	// SE050
	unsafe { pac.P0.outclr.write(|w| w.bits(1u32 << 20)); } // ENABLE
	unsafe { pac.P0.pin_cnf[22].write(|w| w.bits(3) ); }	// SCL
	unsafe { pac.P0.outclr.write(|w| w.bits(1u32 << 22)); }
	unsafe { pac.P0.pin_cnf[24].write(|w| w.bits(3) ); }	// SDA
	unsafe { pac.P0.outclr.write(|w| w.bits(1u32 << 24)); }
	// disconnect GPIOs
	// POWER.SYSTEMOFF <= 1
