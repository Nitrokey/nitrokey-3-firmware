...
impl QspiFlash {
    ...
    pub fn new(
        ...
        obj.qspi
            .psel
            .sck
            .write(|w| unsafe { w.bits(obj.clk_pin.psel_bits()) });
        obj.qspi
            .psel
            .csn
            .write(|w| unsafe { w.bits(obj.cs_pin.psel_bits()) });
        obj.qspi
            .psel
            .io0
            .write(|w| unsafe { w.bits(obj.io0_pin.psel_bits()) });
        obj.qspi
            .psel
            .io1
            .write(|w| unsafe { w.bits(obj.io1_pin.psel_bits()) });
        obj.qspi.ifconfig0.write(|w| {
            w.readoc()
                .fastread()
                .writeoc()
                .pp()
        ...
        obj.qspi
            .ifconfig1
            .write(|w| unsafe { w.sckfreq().bits(15).spimode().mode0().sckdelay().bits(2) });
    ...
    pub fn read_jedec_id(&mut self) -> [u8; 3] {
        ...
        self.qspi.cinstrconf.write(|w| unsafe {
            w.opcode()
                .bits(0x9F)
                .length()
                ._4b()
...
impl littlefs2::driver::Storage for QspiFlash {
    ...
    fn read(&mut self, off: usize, buf: &mut [u8]) -> Result<usize, littlefs2::io::Error> {
        ...
        self.qspi
            .read
            .dst
            .write(|w| unsafe { w.bits(bufptr as u32) });
        self.qspi
            .read
            .cnt
